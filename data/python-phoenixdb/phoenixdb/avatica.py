...
class AvaticaClient(object):
    ...
    def _apply(self, request_data, expected_response_type=None, keepalive_callback=None):
        ...
        try:
            ...
            if keepalive_callback != None:
                while True:
                    print "pre select"
                    readable, _, _ = select.select([self.connection.sock], [], [], 0.5)
                    ...
                    if readable:
                        ...
                        if keepalive_callback() == False:
                            print "Terminating"
                            e = Exception()
                            raise errors.InterfaceError("RPC request terminated", cause=e)
...
#        logger.debug("Received response\n%s", pprint.pformat(response_data))
    ...
    def prepareAndExecute(self, connectionId, statementId, sql, maxRowCount=-1, keepalive_callback=None):
        ...
        return self._apply(request, 'Service$ExecuteResponse', keepalive_callback=keepalive_callback)['results']
